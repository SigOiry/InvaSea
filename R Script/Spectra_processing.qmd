...
```{r SAJA_field}
Saja_df <- read.csv("D:/Git_repo/InvaSea/Data/Spain/Gracilaria-Saja-reflectance.txt", sep="") %>% 
...
ggplot(Saja_df)+
  geom_line(aes(x = Wavelength, y = mean_STD, group = ID))
```
...
write_csv(df_spectra, paste0("Output/",unique(df_spectra$dataset_Name),"_STD.csv"))


```

```{r SAJA_lab}
Saja_df <- read.csv("D:/Git_repo/InvaSea/Data/Spain/SAJA-reflectance.txt", sep="") %>% 
...
Saja_df %>% 
  ggplot(aes(x = Wavelength, y = mean_RAW, group = ID, color = ID))+
  geom_line(show.legend = F)

```

```{r Vigo_lab}
Vigo_df <- read.csv("D:/Git_repo/InvaSea/Data/Spain/CESANTES-reflectance.txt", sep="") %>% 
  pivot_longer(-Wavelength, names_to = "Spectra", values_to = "Value") %>%
  dplyr::filter(Wavelength >= 400 & Wavelength <= 900) %>% 
  group_by(Spectra) %>% 
  mutate(std = (Value-min(Value))/(max(Value)-min(Value))) %>% 
