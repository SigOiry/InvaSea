...
unique(df_long$dataset_Name)


test <- df_long %>% 
  dplyr::filter(dataset_Name == "MORB")
...
for(i in 1:length(notebooks)){
  
  nb <- read.delim(notebooks[i])
...
CSV_sites <- list.files("./Data/CSV", full.names = T) %>% 
  ...
  filter(!name %in% c("all_spectra","RAW_BER_Spectra","RAW_VAN_Spectra","RAW_VIE_Spectra"))
...
for(i in 1:nrow(CSV_sites)){
  ...
  for(ii in 1:length(unique(df$Sample))){
    ...
    plt <- ggplot(a)+
      geom_line(aes(x = Wavelength, y=ref, group = Name, color = Letter))+
